...
param share_urban >= 0, <= 1; # %_urban [-]: share of urban mobility
param share_mobility_public_urban_min >= 0, <= 1; # %_public,min [-]: min limit for penetration of public mobility over urban mobility 
param share_mobility_public_urban_max >= 0, <= 1; # %_public,max [-]: max limit for penetration of public mobility over urban mobility 
param share_mobility_active_urban_min >= 0, <= 1; # %_active,min [-]: min limit for penetration of active mobility over urban mobility
param share_mobility_active_urban_max >= 0, <= 1; # %_active,max [-]: max limit for penetration of active mobility over urban mobility
param share_mobility_public_rural_min >= 0, <= 1; # %_public,min [-]: min limit for penetration of public mobility over rural mobility
param share_mobility_public_rural_max >= 0, <= 1; # %_public,max [-]: max limit for penetration of public mobility over rural mobility
param share_mobility_active_rural_min >= 0, <= 1; # %_active,min [-]: min limit for penetration of active mobility over rural mobility
param share_mobility_active_rural_max >= 0, <= 1; # %_active,max [-]: max limit for penetration of active mobility over rural mobility
...
var Share_mobility_active_urban, >= share_mobility_active_urban_min, <= share_mobility_active_urban_max; # %_Active: Ratio [0; 1] active mobility over total passenger mobility
var Share_mobility_public_rural, >= share_mobility_public_rural_min, <= share_mobility_public_rural_max; # %_Public: Ratio [0; 1] public mobility over total passenger mobility
var Share_mobility_active_rural, >= share_mobility_active_rural_min, <= share_mobility_active_rural_max; # %_Active: Ratio [0; 1] active mobility over total passenger mobility
...
subject to end_uses_t {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [l, h, td] = (if l == "ELECTRICITY" 
		then
			...
			(end_uses_input["MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * share_urban * Share_mobility_public_urban
		else (if l == "MOB_PRIVATE_NA_URBAN" then
			(end_uses_input["MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * share_urban * (1 - Share_mobility_public_urban - Share_mobility_active_urban)
		else (if l == "MOB_PRIVATE_A_URBAN" then
			(end_uses_input["MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * share_urban * (Share_mobility_active_urban)
		else (if l == "MOB_PUBLIC_RURAL" then
			(end_uses_input["MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * (1 - share_urban) * Share_mobility_public_rural
		else (if l == "MOB_PRIVATE_NA_RURAL" then
			(end_uses_input["MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * (1 - share_urban) * (1 - Share_mobility_public_rural - Share_mobility_active_rural)
		else (if l == "MOB_PRIVATE_A_RURAL" then
			(end_uses_input["MOBILITY_PASSENGER"] * mob_pass_time_series [h, td] / t_op [h, td]  ) * (1 - share_urban) * (Share_mobility_active_rural)
